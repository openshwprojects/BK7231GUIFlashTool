...
public class YModem2
{
    ...
    public YModem2(SerialPort port)
    {
        this.port = port;
    }
    ...
    public int send(Stream data_stream, string data_name, long data_size, bool packet_size_16k = true, int retry = 20, object callback = null)
    {
        ...
        Console.WriteLine("YModem2::send: v2 will wait for CRC...");
        if (wait_for_next(CRC) != 0)
        {
            Console.WriteLine("YModem2::send: v2 wait_for_next CRC failed!");
            return -1;
        }
        Console.WriteLine("YModem2::send: v2 wait_for_next CRC ok!");
      ...
      //  Console.WriteLine("YModem2::send: first packet " + BitConverter.ToString(data_for_send).Replace("-", " "));
        port.Write(data_for_send, 0, data_for_send.Length);

        Console.WriteLine("YModem2::send: v2 will wait for ACK...");
        if (wait_for_next(ACK) != 0)
        {
            Console.WriteLine("YModem2::send: v2 wait_for_next ACK failed!");
            return -1;
        }
        Console.WriteLine("YModem2::send: v2 wait_for_next ACK ok!");

        Console.WriteLine("YModem2::send: v2 will wait for second CRC...");
        if (wait_for_next(CRC) != 0)
        {
            Console.WriteLine("YModem2::send: v2 wait_for_next second CRC failed!");
            return -1;
        }
        Console.WriteLine("YModem2::send: v2 wait_for_next second CRC ok!");
        ...
        Console.WriteLine("YModem2::send: sending loop done!");
